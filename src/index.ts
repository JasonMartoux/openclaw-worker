...
function validateRequiredEnv(env: MoltbotEnv): string[] {
  ...
  // Check for AI Gateway or direct API key configuration
  ...
  } else if (!env.ANTHROPIC_API_KEY && !env.MINIMAX_API_KEY) {
    // Direct API access requires at least one API key
    missing.push('ANTHROPIC_API_KEY, MINIMAX_API_KEY, or AI_GATEWAY_API_KEY');
  }
